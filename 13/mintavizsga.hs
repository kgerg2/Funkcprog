...
-- 1
rightmost :: Tree a -> a
rightmost = foldl (\a b -> b) undefined

-- 2
mostCommon :: Eq a => Tree a -> a
mostCommon = getMax . foldr acc [] where
  getMax :: [(Int, a)] -> a
  getMax = snd . foldr (\(i, a) (j, b) -> if i >= j then (i, a) else (j, b)) (0, undefined)
  acc :: Eq a => a -> [(Int, a)] -> [(Int, a)]
  acc a [] = [(1, a)]
  acc a (x@(n, b):xs)
    | a == b    = (n+1, b):xs
    | otherwise = x : acc a xs
...
labelWithAlphabet t = evalState (traverse go t) ['a'..] where
  go a = do
    l <- get
    case l of
      (c:_) -> modify tail *> pure (a, c)
      _     -> undefined -- would cause warning otherwise

-- 3
lookupReplace :: Eq k => [(k,v)] -> Tree k -> Maybe (Tree v)
lookupReplace l t = sequence $ evalState (traverse go t) l where
  go k = do
    l <- get
    pure $ lookup k l
...
keywords :: [String]
keywords = ["not", "and", "while", "do", "if", "end", "true", "false", "exit", "assert"]
...
newScope ma = do
  ...
  a <- ma
  modify (\(env', b) -> (drop (length env' - length env) env', b))
  pure a
...
evalProg []     = pure 0
-- evalProg [x]    = evalSt x
evalProg (x:xs) = evalSt x >> evalProg xs


-- interpreter
--------------------------------------------------------------------------------

runProg :: Bool -> String -> (Int, Env)
runProg b str = case runParser pSrc str of
  Nothing     -> error "parse error"
  Just (p, _) -> runState (evalProg p) ([], b)
