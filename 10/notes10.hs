...
pLetKeyword' :: Parser ()
pLetKeyword' = lexeme pLetKeyword
...
pVarOrParens =  Var <$> pIdent'
            <|> char' '(' *> pExpr <* char' ')'

pApp :: Parser Expr
-- pApp = sepBy1 pExpr ws >>= \l -> pure $ foldl1 App l
pApp = foldl1 App <$> some pVarOrParens

pLet :: Parser Expr 
pLet = Let <$> (pKeyword' "let" *> pIdent') <*> (char' '=' *> pExpr) <*> (pKeyword' "in" *> pExpr)

pLam :: Parser Expr 
pLam = Lam <$> (char' '\\' *> pIdent') <*> (string' "->" *> pExpr)

pExpr :: Parser Expr
pExpr = pLam <|> pLet <|> pApp

main = print $ runParser pExpr "let x = y in z"