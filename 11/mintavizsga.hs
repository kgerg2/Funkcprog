...
instance Foldable Tree where
  ...
  foldr f b (Node2 t1 t2) = foldr f (foldr f b t1) t2
  foldMap = undefined

rightmost :: Tree a -> a
rightmost = foldr1 (\a b -> a)

findElem :: (a -> Bool) -> Tree a -> Maybe a
findElem f = foldr (\a b -> b <|> if f a then Just a else Nothing) Nothing
...
numberElems t = evalState (go t) 0 where
  ...
  go (Node1 a  t ) = (do
    ...
    put (n+1)
    pure $ Node1 (a, n)) <*> go t
  go (Node2 t1 t2) = Node2 <$> go t1 <*> go t2
...
replace l t = evalState (go t) l where
  go (Leaf  a)     = do
    l <- get
    case l of
      (x:xs) -> modify tail *> pure (Leaf x)
      []     -> pure (Leaf a)
  go (Node1 a  t ) = (do
    l <- get
    case l of
      (x:xs) -> modify tail *> pure (Node1 x)
      []     -> pure (Node1 a)) <*> go t
  go (Node2 t1 t2) = Node2 <$> go t1 <*> go t2
